...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=nkirui2030/multi-server:$SHA
kubectl set image deployments/client-deployment client=nkirui2030/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=nkirui2030/multi-worker:$SHA
